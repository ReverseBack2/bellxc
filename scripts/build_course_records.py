...
def canonical_race_name(name):
    ...
    if re.fullmatch(r"Crystal Springs (?:Invitational|Invite)", value, re.I):
        return "Serra/Crystal Springs Invitational"
...
payload = {
    ...
    "methodology": "Top 10 lists are grouped by canonical race name, exact XCStats course name, and normalized race distance, so different meets at the same venue are not mixed while known naming variants of the same meet are merged. WCAL 3 and WCAL Finals are treated as the same meet. Serra Invitational and Crystal Springs Invitational/Invite are treated as the same meet under Serra/Crystal Springs Invitational. Equivalent 5K and 3.1-mile labels are grouped together. Each course configuration includes the years it appears in the XCStats archive. Each runner appears once per list with his fastest qualifying performance. Freshman/Sophomore class records use grade at race regardless of squad. Baylands Invitational, CCS, and State use Freshman, Sophomore, Junior, and Senior class lists. JV/Varsity lists are used for other meets from the XCStats Squad field.",
    "courses": configs,
}
