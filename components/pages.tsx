...
export function UsefulLinksPage() {
  return <div className="content-page"><PageHead eyebrow="Runner resources" title="Useful links" intro="Schedules, history, live results, and key information for runners and families." /><section className="link-section"><h2>Team & race resources</h2><div className="resource-grid">{usefulLinks.map((item) => <a key={item.name} href={item.href} target="_blank" rel="noreferrer"><strong>{item.name}</strong><p>{item.description}</p><ArrowUpRight /></a>)}</div></section><section className="link-section"><h2>Live timing</h2><p className="section-intro">The timing provider changes by meet. The event will usually post which live-results service is being used.</p><div className="resource-grid compact">{timingLinks.map((item) => <a key={item.name} href={item.href} target="_blank" rel="noreferrer"><Clock3 /><strong>{item.name}</strong><p>{item.description}</p><ArrowUpRight /></a>)}</div></section><aside className="callout"><span>California safety policy</span><div><p><strong>San Jose example:</strong> An air temperature of 91°F with 20% relative humidity does not mean the WBGT is 91°F. Dry air allows much more evaporative cooling, so the wet-bulb temperature can be about 64°F.</p><p><strong>Illustrative breakdown:</strong> 91°F air · 20% relative humidity · ~64°F wet-bulb · roughly 74–76°F estimated direct-sun WBGT. The WBGT estimate can move with wind, cloud cover, sun angle, and solar load.</p><p>Bellarmine is a <strong>CIF Category 1</strong> school. Under the 2026–27 CIF policy, Category 1 is green below 76.1°F; restrictions increase above that, and no outdoor workouts or contests are allowed at 86.2°F or higher. Actual practice decisions should use an on-site WBGT reading whenever possible.</p></div><div><a href="https://cifss.org/wp-content/uploads/2026/08/EXTREME_HEAT_AND_AIR_QUALITY_POLICY.pdf" target="_blank" rel="noreferrer">Current CIF policy <ArrowUpRight /></a><a href="https://cifss.org/wp-content/uploads/2026/08/WBGT_Category.pdf" target="_blank" rel="noreferrer">Bellarmine WBGT category <ArrowUpRight /></a><a href="https://www.weather.gov/arx/wbgt" target="_blank" rel="noreferrer">How WBGT works <ArrowUpRight /></a></div></aside></div>;
}
